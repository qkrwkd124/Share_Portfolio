<template>
  <v-layout mt-5 wrap>
    <v-flex v-for="i in portfolios.length > limits ? limits : portfolios.length" xs12 md4 sm6>
	<router-link :to="{ path: '/detail', query: { nid: i }}">
...
export default { 
	...
	methods: {
		async getPortfolios() {
			...
			this.$emit("portfolionum", this.portfolios.length)
		},
		loadMorePortfolios() {
